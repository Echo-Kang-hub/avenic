...
test("the launch question names the methods this project already has, and only reads", async () => {
  ...
  try {
    ...
    // been written.
    const nothing = await launchMethodReadiness(root, "claude");
    ...
    // than offer two identical-looking choices.
    await writeApiConfiguration(root, "claude", "project", {
      provider: "FixtureProvider",
      baseUrl: "https://provider.fixture.invalid/v1",
      model: "fixture-model",
      credential: "fixture-credential-not-a-real-secret",
    });
    const both = await launchMethodReadiness(root, "claude");
    ...
    assert.equal(informed.options[1].description, "use a provider/model/API configuration — .claude/settings.local.json is already written");
    ...
    await writeFile(path.join(home, ".credentials.json"), "{ not json\n");
    assert.deepEqual(await launchMethodReadiness(root, "claude"), { account: false, api: ".claude/settings.local.json" });
    ...
    assert.equal(await readFile(path.join(home, ".credentials.json"), "utf8"), "{ not json\n");
...
test("a configuration edited out of the file is not a method a launch has", async () => {
  ...
  try {
    ...
    // 用户在 Avenic 之外删掉了配置块：文件还在、账本还在，但这次启动不会带着任何
    // API 配置跑起来 —— 问题里就不能再把它说成一种可用的方法（那正是会误导人的
    // 那一次回答：用户选了它，agent 却起在没有任何 provider 的状态上）。
    ...
    // 启动本身也要说得出为什么：不是「Avenic 没写过」，是「写过的那份现在不在了」。
    const runtime = await resolveEffectiveAgentRuntime(root, "claude", { environment });
    assert.match(runtime.note ?? "", /no longer holds the API configuration Avenic wrote/);
  } finally {
    await rm(root, { recursive: true, force: true });
    await rm(home, { recursive: true, force: true });
  }
