...
//
// Exit code is 0 only when every compared edge is inside its tolerance *and*
// there was something to compare: a fixture that lists no anchors, a dump none
// of them are in, a dump measured at another viewport than the reference's, and
// an anchor whose `ref` names no edge at all are all failures, so it can gate a
// release the same way the test suite does.
//
...
lines.push(problems.length === 0
  ? `PASS — every compared edge is within tolerance.`
  : `FAIL — ${problems.join("; ")}`);
const report = lines.join("\n");

const stem = geometryPath.replace(/\.geometry\.json$/, "");
writeFileSync(`${stem}.compare.md`, `${report}\n`, "utf8");
writeFileSync(`${stem}.compare.json`, `${JSON.stringify({ viewport: geometry.viewport, requested, compared, passed, gatedAnchors, reportOnly: reportOnly.map((row) => row.name), noEdges: edgeLess.map((row) => row.name), missing: missing.map((row) => row.name), problems, failures: failures.map((row) => row.name), rows }, null, 1)}\n`, "utf8");

// The console view is the same table without the markdown pipes — the verdict's
// own reasons first, then one line per failing anchor, so a broken layout is
...
console.log(`edges ${passed}/${compared} ok · ${rows.length} anchors (${gatedAnchors} gated${reportOnly.length === 0 ? "" : `, ${reportOnly.length} report-only`}) · report: ${path.relative(process.cwd(), `${stem}.compare.md`)}`);
